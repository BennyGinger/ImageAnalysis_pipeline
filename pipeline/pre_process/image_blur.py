...
def blur_img(exp_obj_lst: list[Experiment], blur_kernel: list[int], blur_sigma: int, img_fold_src: PathLike="", blur_overwrite: bool = False)-> None:
    ...
    for exp_obj in exp_obj_lst:
        ...
        fold_src,img_list = img_list_src(exp_obj, img_fold_src)
        ...
        # Save settings
        exp_obj.preprocess.img_blured = [f"blur_kernel={blur_kernel}",f"blur_sigma={blur_sigma}",f"fold_src={fold_src}"]
