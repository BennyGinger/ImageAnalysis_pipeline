...
def get_tmats_chan(stackreg: StackReg, exp_set: Experiment, reg_channel: str)-> dict[str,np.ndarray]:
    ...
    # Load ref image
    img_ref = load_stack(exp_set.raw_imgs_lst,reg_channel,0,True)
    # Get the list of channel to process
    channel_lst = [chan for chan in exp_set.active_channel_list if chan!=reg_channel]
    # Get all the tmats
    tmats_dict = {}
    for chan in channel_lst:
        img = load_stack(exp_set.raw_imgs_lst,chan,0,True)
        tmats_dict[chan] = stackreg.register(img_ref,img)
    return tmats_dict

def apply_chan_shift(exp_set: Experiment, stackreg: StackReg, reg_channel: str)-> None:
    """Apply the channel shift correction to the images."""
    # Get all the tmats
    tmats_dict = get_tmats_chan(stackreg,exp_set,reg_channel)
    
    # Sort the images by channel, expcept the reg_channel, as it doesn't need to be processed
    sorted_channels = {chan: [file for file in exp_set.raw_imgs_lst if chan in file] 
                      for chan in exp_set.active_channel_list if chan!=reg_channel} 
...
def get_tmats_first(stackreg: StackReg, exp_set: Experiment, reg_channel: str)-> dict[int,np.ndarray]:
    ...
    frame_range = range(1,exp_set.img_properties.n_frames) 
    # Get all the tmats
    tmats_dict = {}
    for frame in frame_range:
        img = load_stack(exp_set.raw_imgs_lst,reg_channel,frame,True)
        tmats_dict[frame] = stackreg.register(img_ref,img)
    return tmats_dict

def get_tmats_mean(stackreg: StackReg, exp_set: Experiment, reg_channel: str)-> dict[int,np.ndarray]:
    ...
    img_ref = np.mean(load_stack(exp_set.raw_imgs_lst,reg_channel,range(exp_set.img_properties.n_frames),True),axis=0)
    # Get the frame range, that is all the frames
    frame_range = range(exp_set.img_properties.n_frames)
    # Get all the tmats
    tmats_dict = {}
    for frame in frame_range:
        img = load_stack(exp_set.raw_imgs_lst,reg_channel,frame,True)
...
def get_tmats_previous(stackreg: StackReg, exp_set: Experiment, reg_channel: str)-> dict[int,np.ndarray]:
    """Register all frames of the given channel compared to the previous frame.
    Output is a dict with the frame (excluding the first one) as key and the tmat np.ndarray (2D) as value."""
    tmats_dict = {}
    for frame in range(1,exp_set.img_properties.n_frames):
        # Load ref image
        if frame==1: # For the second frame, use the first frame as ref
            img_ref = load_stack(exp_set.raw_imgs_lst,reg_channel,frame-1,True)
...
def apply_frame_shift(stackreg: StackReg, exp_set: Experiment, reg_channel: str, img_ref: str)-> None:
    """Apply the frame shift correction to the images depending on the img_ref (first, mean or previous)."""
    # Load ref image
    if img_ref=='first':
        tmats_dict = get_tmats_first(stackreg,exp_set,reg_channel)
    elif img_ref=='mean':
        tmats_dict = get_tmats_mean(stackreg,exp_set,reg_channel)
    elif img_ref=='previous':
        tmats_dict = get_tmats_previous(stackreg,exp_set,reg_channel)
    
    # Sort the images by frame
    sorted_frames = {frame: [file for file in exp_set.raw_imgs_lst if f"_f{frame+1:04d}" in file] 
