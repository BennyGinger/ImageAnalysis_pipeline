import pytest
import numpy as np
from mask_transformation.mask_morph import copy_first_last_mask, get_masks_to_morph_lst, fill_gaps
...
def test_get_masks_to_morph_lst(mask_stack):
    ...
    expected_output = []
    assert sorted(get_masks_to_morph_lst(mask_stack)) == sorted(expected_output)

    # Test case 2: Single gap between masks
    mask = mask_stack.copy()
    mask[2:4, :, :] = 0
    expected_output = [(2, 4, 2)]
    assert sorted(get_masks_to_morph_lst(mask)) == sorted(expected_output)
    ...
    mask[:2, :, :] = 0
    expected_output = []
    assert get_masks_to_morph_lst(mask) == expected_output
