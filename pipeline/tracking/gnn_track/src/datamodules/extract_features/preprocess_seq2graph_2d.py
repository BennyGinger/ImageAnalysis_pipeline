...
def create_csv(input_images, input_masks, input_seg,
    ...
    for seq in sequences:
        curr_img_path = os.path.join(input_images, seq + "/Image_Raw")
        curr_msk_path = os.path.join(input_masks, seq + "/Masks_Tracked")
        curr_seg_path = os.path.join(input_seg, seq + "/Masks_Segmented")
        ds = TestDataset(
            path=curr_img_path,
            ...
            sec_path=seq)
        if basic:
            ds.preprocess_basic_features(path_to_write=output_csv)
        else:
            ds.preprocess_features_metric_learning(path_to_write=output_csv, dict_path=input_model)


# def create_csv(folder_path, input_model, output_csv, basic=False, sequences=['01', '02']):
#     for seq in sequences:
#         curr_img_path = os.path.join(folder_path, seq + "/Image_Raw")
#         curr_tra_path = os.path.join(folder_path, seq + "/Masks_Tracked")
#         curr_seg_path = os.path.join(folder_path, seq + "/Masks_Segmented")
#         ds = TestDataset(
#             dir_img=curr_img_path,
#             dir_tra_masks=curr_tra_path,
#             dir_seg_masks=curr_seg_path,
#             type_img="tif",
#             seq_path=seq)
#         if basic:
#             ds.preprocess_basic_features(path_to_write=output_csv)
