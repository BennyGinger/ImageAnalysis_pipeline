...
class TestDataset(Dataset):
    def __init__(self,
        ...
        if len(self.images) != len(self.masks) or len(self.images) != len(self.results):
            ...
            self.masks = self.masks[:min_frames]
            print(
                f"Pay attention! the images , mask and results are not with the same number "
                f"len_img = {len_img}, len_masks = {len_masks}, len_res = {len_res} ")
            for img_path, seg_path, res_path in zip(self.images[:min_frames], self.masks[:min_frames], self.results[:min_frames]):
                
                img_name = Path(img_path).stem
                im_num = re.findall(r'\d+', img_name)[-1]
                # im_num = img_path.split(".")[-2][-3:]

                seg_name = Path(seg_path).stem
                mask_num = re.findall(r'\d+', seg_name)[-1]
                # mask_num = seg_path.split(".")[-2][-3:]

                res_name = Path(res_path).stem
                res_num = re.findall(r'\d+', res_name)[-1]
                # res_num = res_path.split(".")[-2][-3:]    
                
                assert im_num == mask_num, f"Image number ({im_num}) is not equal to mask number ({mask_num})"
                assert im_num == res_num, f"Image number ({im_num}) is not equal to result number ({res_num})"
    ...
    def preprocess_basic_features(self, path_to_write):
        ...
        for ind_data in range(self.__len__()):
            ...
            for ind, id_res in enumerate(np.unique(result)):
                ...
                df.loc[row_ind, "min_depth_bb"], df.loc[row_ind, "min_row_bb"], df.loc[row_ind, "min_col_bb"], \
                df.loc[row_ind, "max_depth_bb"], df.loc[row_ind, "max_row_bb"], df.loc[row_ind, "max_col_bb"] = properties.bbox
    ...
    def preprocess_features_metric_learning(self, path_to_write, dict_path):
        ...
        for ind_data in range(self.__len__()):
            ...
            for ind, id_res in enumerate(np.unique(result)):
                ...
                # extracting statistics using regionprops
                properties = regionprops(np.uint8(result == id_res), img)[0]
                embedded_feat = self.extract_freature_metric_learning(properties.bbox, img.copy(), result, id_res)
